...
func logFood()->Food?{
    guard let foodName = foodTextField.text , let calories = Int(caloriesTextField.text!) else {
        return nil
    }
    let foodObject = Food(name: foodTextField.text!, calories: Int(caloriesTextField.text!))
    return foodObject
}
...
let captureReturnValue = logFood()
print("Food Name:\t\(captureReturnValue?.name ?? "not found")")
print("Food calories:\t\(captureReturnValue?.calories ?? 0)")
